...
struct virtual_interface * create_alias(struct nl_sock *sock, struct physical_interface *iff, struct interface *p, List *virt_list);
/*
 *
 */
				...
				if(phys->virt_list){
					list_for_each(item, phys->virt_list){
						...
						for (j = list_size(virt_list); j--; ) {
							if(iff == (struct virtual_interface *)list_get(virt_list, j)->data){
								Litem *tmpitem = list_remove(virt_list, j);
								free(tmpitem->data);
								free(tmpitem);
								break;
							}
...
int
add_virt_for_diss(struct nl_sock *sock, struct physical_interface *phy, List *iff_list, List *virt_list)
{
	if(!phy->diss) return 0;
	...
	list_for_each(item, iff_list){
		...
		if(p->gateway) {
			...
			struct virtual_interface *v = create_alias(sock, phy, (struct interface*)p, virt_list);
...
struct virtual_interface *
create_alias(struct nl_sock *sock, struct physical_interface *iff, struct interface *p, List *virt_list)
{
	Litem *item = 0;
