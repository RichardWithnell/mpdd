...
static int start_monitor(struct cache_monitor* mon)
{
    ...
    if ((ret =
             nl_cache_mngr_add(mngr, "route/addr", (change_func_t)&cache_update, mon,
                               &(mon->addr_cache)))) {
        nl_perror(ret, 0);
        pthread_exit(&ret);
    }
    ...
    if ((ret =
             nl_cache_mngr_add(mngr, "route/link", (change_func_t)&cache_update, mon,
                               &(mon->link_cache)))) {
        nl_perror(ret, 0);
        pthread_exit(&ret);
    }
    ...
    if ((ret =
             nl_cache_mngr_add(mngr, "route/route", (change_func_t)&cache_update, mon,
                               &(mon->route_cache)))) {
        nl_perror(ret, 0);
        pthread_exit(&ret);
    }
